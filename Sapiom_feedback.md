...
### 9. `@upstash/redis` client library is incompatible with standard provisioned Redis

- **What happened:** Sapiom provides access to Upstash Redis, but the standard `@upstash/redis` npm package cannot be used with the standard provisioned Redis because authentication goes through x402 (via `@sapiom/fetch`) rather than Upstash's native REST token.
...
- **Actual behavior:** For standard provisioned Redis, you must abandon the Upstash client and write raw HTTP commands through `@sapiom/fetch`, losing all typed methods, automatic serialization, pipeline helpers, and the general developer experience of `@upstash/redis`.
- **Impact:** Medium — ongoing overhead throughout the project. Every Redis operation in `lib/redis.ts` was manual HTTP until we migrated to direct Upstash.
- **Partial mitigation exists:** The Data page documents a "Fixed Redis Plans" section (`POST /v1/redis/databases/fixed`) that returns direct Upstash credentials (`endpoint`, `restToken`, `readOnlyRestToken`), which ARE compatible with `@upstash/redis`. However, these are paid capacity-based plans (starting at ~$10/month equivalent), not the standard provisioned Redis. The standard provisioned Redis (the default path in the docs) has no such escape hatch.
- **Suggestion:** Either make the standard provisioned Redis also return a `restToken` for native client compatibility, or prominently recommend Fixed Redis Plans for applications that need typed client support.
- **Doc source:** [Data](https://docs.sapiom.ai/capabilities/data/) — the Quick Example and Complete Example use standard provisioned Redis with x402 proxy URLs, not Fixed Plans. The phrase *"interact with Redis entirely over REST — no TCP connections or Redis client libraries needed"* hints that native clients aren't the intended path but is ambiguous — "not needed" is not the same as "will not work." The Fixed Redis Plans section exists on the same page but is positioned far below the primary examples; a developer following the Quick Start will use the standard provisioned Redis and only discover the incompatibility at runtime. [@sapiom/fetch](https://docs.sapiom.ai/reference/sdk/fetch/) — explains the x402 payment flow but does not mention that this payment mechanism makes native provider client libraries incompatible with standard provisioned resources. [How Sapiom Works](https://docs.sapiom.ai/how-it-works/) — describes the x402 gateway architecture but does not call out that only raw HTTP through the SDK works for standard provisioned resources.
...
### 12. ~~QStash schedules endpoint is undocumented~~ — RESOLVED

- **Status:** Fixed by Sapiom. The [Messaging](https://docs.sapiom.ai/capabilities/messaging/) page now includes a full "Schedule Management" section documenting create, list, get, delete, pause, and resume endpoints with code examples and cron format documentation. This was undocumented when we first encountered it during the build but has since been addressed.
...
- **Doc source:** [Verify Users](https://docs.sapiom.ai/capabilities/verify/) — this page **explicitly documents email as a supported verification method**. The API Reference table lists `target.type` as `"phone_number" or "email_address"`. A dedicated JSON example shows `{ "target": { "type": "email_address", "value": "user@example.com" } }`. The "How It Works" section states: *"Send a code — Call the /verifications endpoint with a phone number or email."* However, the separate API Reference page [Send verification code](https://docs.sapiom.ai/api-reference/endpoints/verification/v1-services-verify-send-post/) documents an **entirely different request schema**: it uses a flat `{ "phoneNumber": "+15551234567" }` body instead of the `target` object, describes the endpoint as *"Sends a verification code via SMS to the provided phone number"* (no email mention), and points to a different URL path (`/v1/services/verify/send` vs `/verifications`). These two pages describe incompatible schemas for the same operation. In practice, the capability page's schema (`target` object) is the one that works for phone — email returns a 502.
